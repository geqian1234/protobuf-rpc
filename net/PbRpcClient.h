...
public:
    zkcpp::zk_cpp::child_event_handler_t child_event_handler;
    zkcpp::zk_cpp::data_change_event_handler_t data_change_event_handler;
    //远程rpc服务器地址
...
private:
    /*异步连接远程rpc服务器*/
    void connect();
    void handle_rpcserver_address_change(const std::string& path, const std::string& new_value);
    void handle_rpcserver_node_change(const std::string& path, const std::vector<std::string>& new_children);
};
