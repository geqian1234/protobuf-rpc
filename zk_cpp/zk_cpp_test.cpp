...
namespace utils {
    static std::string perms_to_string(int32_t perms) {
        if (perms == zkcpp::zoo_perm_all) {
            return "all";
        }

        std::string ret;
        if (perms & zkcpp::zoo_perm_create) {
            ret.append("c");
        }
        if (perms & zkcpp::zoo_perm_read) {
            ret.append("r");
        }
        if (perms & zkcpp::zoo_perm_delete) {
            ret.append("d");
        }
        if (perms & zkcpp::zoo_perm_write) {
            ret.append("w");
        }
        if (perms & zkcpp::zoo_perm_admin) {
            ret.append("a");
        }
        return ret;
    }
    ...
    static int32_t perms_string_to_int(const std::string& perm_str) {
        if (perm_str == "all") {
            return zkcpp::zoo_perm_all;
        }

        int32_t perms = 0;
        for (auto c : perm_str) {
            if (c == 'c') {
                perms |= zkcpp::zoo_perm_create;
            }
            else if (c == 'r') {
                perms |= zkcpp::zoo_perm_read;
            }
            else if (c == 'd') {
                perms |= zkcpp::zoo_perm_delete;
            }
            else if (c == 'w') {
                perms |= zkcpp::zoo_perm_write;
            }
            else if (c == 'a') {
                perms |= zkcpp::zoo_perm_admin;
            }
        }
        return perms;
    }
...
int main() {
    ...
    do {
        zkcpp::zoo_rc ret = zk.connect(urls);
        if (ret != zkcpp::z_ok) {
            printf("try connect zk server failed, code[%d][%s]\n", 
                ret,zkcpp::zk_cpp::error_string(ret));
            break;
        }
        ...
        while (std::cin >> cmd) {
            if (cmd == "create") {
                ...
                zkcpp::zoo_rc ret = zkcpp::z_ok;
                if (flag == 0) {
                    std::vector<zkcpp::zoo_acl_t> acl;
                    acl.push_back(zkcpp::zk_cpp::create_world_acl(zkcpp::zoo_perm_all));
                ...
                printf("create path[%s] flag[%d] ret[%d][%s], rpath[%s]\n",
                    path.c_str(), flag, ret, zkcpp::zk_cpp::error_string(ret), rpath.c_str());
            }
            else if (cmd == "get") {
                std::string path;
                std::string value;
                ...
                zkcpp::zoo_rc ret = zk.get_node(path.c_str(), value, nullptr, true);
                printf("try get path[%s]'s value, value[%s] ret[%d][%s]\n",
                    path.c_str(), value.c_str(), ret,zkcpp::zk_cpp::error_string(ret));
            }
            else if (cmd == "set") {
                std::string path;
                std::string value;
                ...
                zkcpp::zoo_rc ret = zk.set_node(path.c_str(), value, -1);
               ...
               zkcpp::zoo_rc ret = zk.exists_node(path.c_str(), nullptr, true);
                printf("try_check path[%s] exist[%d], ret[%d][%s]\n",
                    path.c_str(), ret == zkcpp::z_ok, ret, zkcpp::zk_cpp::error_string(ret));
            }
            else if (cmd == "ls") {
                std::string path;
                ...
                zkcpp::zoo_rc ret = zk.get_children(path.c_str(), children, true);
                printf("try get path[%s]'s children's, children count[%d], ret[%d][%s]\n",
                    path.c_str(), (int32_t)children.size(), ret, zkcpp::zk_cpp::error_string(ret));
                ...
                zkcpp::zoo_rc ret = zk.delete_node(path.c_str(), -1);
                printf("try delete path[%s], ret[%d][%s]\n",
                    path.c_str(), ret,zkcpp::zk_cpp::error_string(ret));
            }
            else if (cmd == "setacl") {
                ...
                if (scheme == "world") {
                    ...
                    auto ac = zkcpp::zk_cpp::create_world_acl(perms);
                    acl.push_back(ac);
                }
                else if (scheme == "auth") {
                    // "id" is empty
                    ...
                    auto ac = zkcpp::zk_cpp::create_auth_acl(perms);
                    acl.push_back(ac);
                }
                    ...
                    perms = utils::perms_string_to_int(perm_str);
                 auto ac = zkcpp::zk_cpp::create_digest_acl(perms, user_name, passwd);
                    acl.push_back(ac);
                }
                else if (scheme == "ip") {
                    const std::string& id = splits[1];
                    const std::string& perm_str = splits[2];
                    perms = utils::perms_string_to_int(perm_str);
                    auto ac = zkcpp::zk_cpp::create_ip_acl(perms, id);
                    acl.push_back(ac);
                }
                else {
                ...
                std::vector<zkcpp::zoo_acl_t> acl;
                zkcpp::zoo_rc rt = zk.get_acl(path.c_str(), acl);
                printf("get acl of path[%s], ret[%d][%s], acl_count[%d]\n",
                    path.c_str(), ret, zkcpp::zk_cpp::error_string(ret), (int32_t)acl.size());
                ...
                zkcpp::zoo_rc rt = zk.add_auth(user_name, user_passwd);
                ...
                zkcpp::zoo_rc ret = zk.watch_data_change(path.c_str(), data_change_event, &value);
                ...
                zkcpp::zoo_rc ret = zk.watch_children_event(path.c_str(), child_change_events, &children);
